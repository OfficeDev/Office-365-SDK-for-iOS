/*******************************************************************************
**NOTE** This code was generated by a tool and will occasionally be
overwritten. We welcome comments and issues regarding this code; they will be
addressed in the generation tool. If you wish to submit pull requests, please
do so for the templates in that tool.

This code was generated by Vipr (https://github.com/microsoft/vipr) using
the T4TemplateWriter (https://github.com/msopentech/vipr-t4templatewriter).

Copyright (c) Microsoft Open Technologies, Inc. All Rights Reserved.
Licensed under the Apache License 2.0; see LICENSE in the source repository
root for authoritative license information.﻿
******************************************************************************/



#ifndef MSGRAPHITEMATTACHMENTFETCHER_H
#define MSGRAPHITEMATTACHMENTFETCHER_H

#import "MSGraphModels.h"
#import "api/api.h"
#import "core/core.h"
#import "core/MSOrcEntityFetcher.h"

@class MSGraphOutlookItemFetcher;
@class MSGraphOutlookItemFetcher;
@class MSGraphItemAttachmentOperations;


/** MSGraphItemAttachmentFetcher
 *
 */
@interface MSGraphItemAttachmentFetcher : MSOrcEntityFetcher

@property (copy, nonatomic, readonly) MSGraphItemAttachmentOperations *operations;

- (instancetype)initWithUrl:(NSString*)urlComponent parent:(id<MSOrcExecutable>)parent;
- (void)readWithCallback:(void (^)(MSGraphItemAttachment *, MSOrcError *))callback;
- (void)update:(MSGraphItemAttachment *)itemAttachment callback:(void (^)(MSGraphItemAttachment *, MSOrcError*))callback ;
- (void)delete:(void(^)(int status, MSOrcError *))callback;
- (MSGraphItemAttachmentFetcher *)addCustomParametersWithName:(NSString *)name value:(id)value;
- (MSGraphItemAttachmentFetcher *)addCustomHeaderWithName:(NSString *)name value:(NSString *)value;
- (MSGraphItemAttachmentFetcher *)select:(NSString *)params;
- (MSGraphItemAttachmentFetcher *)expand:(NSString *)value;

@property (strong, nonatomic, readonly, getter=item) MSGraphOutlookItemFetcher *item;

@end

#endif
