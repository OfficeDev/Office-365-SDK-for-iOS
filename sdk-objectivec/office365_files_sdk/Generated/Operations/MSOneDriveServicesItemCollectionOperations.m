...
/**
* The implementation file for type MSOneDriveServicesItemCollectionOperations.
*/

@implementation MSOneDriveServicesItemCollectionOperations

- (NSURLSessionTask *)addWithPath:(NSString *)path nameConflict:(NSString *)nameConflict type:(NSString *)type content:(NSStream *)content callback:(void (^)(MSOneDriveServicesItem *item, MSODataException *error))callback {
	...
	NSURLSessionTask *task = [self addRawWithPath:pathString nameConflict:nameConflictString type:typeString content:contentString callback:^(NSString *returnValue, MSODataException *exception) {
	   ...
	   if (exception == nil) {
			...
			MSOneDriveServicesItem * result = (MSOneDriveServicesItem *)[super.resolver.jsonSerializer deserialize:[returnValue dataUsingEncoding:NSUTF8StringEncoding] asClass:[MSOneDriveServicesItem class]];
...
- (NSURLSessionTask *)getByPathWithPath:(NSString *)path callback:(void (^)(MSOneDriveServicesItem *item, MSODataException *exception))callback {
	...
	NSURLSessionTask *task = [super oDataExecuteRequest:request callback:^(id<MSODataResponse> response, MSODataException *exception){
       ...
       if (exception == nil) {
			...
			MSOneDriveServicesItem * result = (MSOneDriveServicesItem *)[super.resolver.jsonSerializer deserialize:response.data asClass:[MSOneDriveServicesItem class]];
            callback(result, exception);
        }
        else {
