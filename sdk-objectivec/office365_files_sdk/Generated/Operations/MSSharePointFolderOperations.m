...
-(NSURLSessionTask*) copy : (NSString *) destFolderId : (NSString *) destFolderPath : (NSString *) newName :  (void (^)(MSSharePointFolder *folder, MSODataException *error)) callback;{
	...
	NSURLSessionTask* task = [self copyRaw 	: destFolderIdString: destFolderPathString: newNameString :^(NSString *returnValue, MSODataException *error){
       
	   if(error == nil){
			MSSharePointFolder * result = (MSSharePointFolder *)[[[self getResolver]getJsonSerializer] deserialize:[returnValue dataUsingEncoding:NSUTF8StringEncoding] : [MSSharePointFolder class]];
