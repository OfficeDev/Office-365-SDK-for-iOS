...
/**
* The implementation file for type MSOSharePointFileOperations.
*/

@implementation MSOSharePointFileOperations
...
-(NSURLSessionDataTask*)copy : (NSString *) destFolderId : (NSString *) destFolderPath : (NSString *) newName : (void (^)(MSOSharePointFile *file, NSError *error))callback{
	...
	NSURLSessionDataTask* task = [super oDataExecuteForPath:url withContent : payload andMethod:POST andCallback:^(id<MSODataResponse> r, NSError *error) {
       ...
       if(error == nil){
			MSOSharePointFile * result = (MSOSharePointFile *)[[[self getResolver]getJsonSerializer] deserialize:[r getData] : [MSOSharePointFile class]];
